...
#aa0 = asp_clustering_it_cl(G,g0,6,m)
...
gamma = [[366154.37744, 4384470.547707],
...
###
	...
	return [dhn_gamma,c[1],c,bom_grans(ggg_gamma,node100)]
	...
	for i in g_gamma.nodes():
		if g_gamma.node[i]['population'] == -1 and g_gamma.degree(i) >= d:
			...
			for j in g_gamma.neighbors(i):
				if g_gamma.node[j]['population'] != -1:
					l_nnn.append(i)
					#print i
	return diff(l_all,l_nnn)
