...
larissa =  nx.DiGraph.to_undirected(nx.read_shp('./aa_scratch0/axons_larissa_with_buildings6_sub0.shp'))#forNetX
...
def netwrk_cost_dummy000(G,g,dhn):
	cutoff_null_leafs0(g)
	g.node[dhn]['root_node'] = 1
	l, gg, leafs, null_l = a_graph_efficiency(g, dhn,1)
	q = a_di_main_paths(G,l)
...
#n_l = [(365978.89560635854, 4384774.3749764515),(366037.97642770055, 4384349.680197536),(366185.16266480635, 4384548.408219551)]

#hyperfinalgraph(G,n_l)


def output100shpfinal(G,gamma,gamma00):
	n_l = [(365978.89560635854, 4384774.3749764515),(366037.97642770055, 4384349.680197536),(366185.16266480635, 4384548.408219551)]
	hyperfinalgraph(G,n_l)
	g_gamma = cluster_to_subready(G,gamma)
	#hyperfinalgraph(g_gamma,n_l)
	##dhn_gamma = find_dhn(G,g_gamma)
	dhn_gamma = (365571.467878, 4384595.998376)
	ggg_gamma, node100, cost100 = netwrk_cost_dummy2(G,g_gamma,dhn_gamma)
	#hyperfinalgraph(ggg_gamma,n_l)
	import os, sys
	# Path to be created
	path = "/home/bluesman/Dropbox/NGNMarcostuff/LarissasPython/aa_scratch/shps/"+str(gamma00)
	os.mkdir(path)
...
def output100shpfinal_not(G,gamma):
	n_l = [(365978.89560635854, 4384774.3749764515),(366037.97642770055, 4384349.680197536),(366185.16266480635, 4384548.408219551)]
	hyperfinalgraph(G,n_l)
	g_gamma = cluster_to_subready(G,gamma)
	dhn_gamma = (365571.467878, 4384595.998376)
	ggg_gamma, node100, cost100 = netwrk_cost_dummy000(G,g_gamma,dhn_gamma)
	#
	for u,v,d in ggg_gamma.edges(data=True):
		if 'ALPHA' in d:
			del d['ALPHA']
		if 'grans' in d:
			del d['grans']
		...
		if 'block_id' in d:
			del d['block_id']
	for n,d in ggg_gamma.nodes(data=True):
		if 'suc2gload' in d:
			del d['suc2gload']
		if 'sucgload' in d:
			del d['sucgload']
...
###############################################################
#mhns_list = dhn_mhns0(G,gamma,best_dhn,deg3,deg2,deg1,0,300,3)
#hyperfinalgraph(G,mhns_list)


#n_l = [(365978.89560635854, 4384774.3749764515),
#(366037.97642770055, 4384349.680197536),
#(366185.16266480635, 4384548.408219551)]
...
def hyperfinalgraph0(ggg_gamma,mhns_list):
	mhns_list0 = []
	for i in mhns_list:
		if i not in ggg_gamma:
			mhns_list0.append(i)
...
###
###best_dhn = d3_cost[0][0] 
#best_dhn = (365571.467878, 43 dhn_mhns0(G,gamma,best_dhn,deg3,deg2,deg1,lmin,lmax,d84595.998376)
#subs_g = subs(gamma,best_dhn)
###
#g0, g1, g2 = subs_g[0], subs_g[1], subs_g[2]
###
...
def alphas():
	...
	mhns_list0 = dhn_mhns0(G,gamma,best_dhn,deg3,deg2,deg1,0,300,3)
	#
	mhns_list = []
	for i in mhns_list0:
	...
	#
	ggg0_gamma, node1000, cost1000 = netwrk_cost_dummy000(G,g0_gamma,dhn_gamma)
	#
	...
	#
	#write_shp(ggg0_gamma,'/home/bluesman/Dropbox/NGNMarcostuff/LarissasPython/aa_scratch/shps/check')
	#
	sl3 = []
	sl2 = []
	sl1 = []
	...
	for n in ggg0_gamma.nodes():
		...
		if ggg0_gamma.node[n]['manhole'] == 3:
			sl3.append(n)
		if ggg0_gamma.node[n]['manhole'] == 0:
			sl0.append(n)
	#'./aa_scratch0/axons_larissa_with_buildings6_sub0.shp'
	write_shp(ggg0_gamma,'./aa_scratch0/shps')
	
	return [sl1,sl2,sl3,sl0]
...
#write_shp(ggg_gamma,'/home/bluesman/Dropbox/NGNMarcostuff/LarissasPython/aa_scratch/shps/one')
...
#NGNMarcostuff/LarissasPython/aa_scratch/aa_scratch0/
